...
- `MONGODB_URI` _(Ensure your MongoDB cluster network access allows IPs `0.0.0.0/0`)_
- `JWT_SECRET`
- `RESEND_API_KEY`
- _(Any other required keys)_
...
4. Set the **Mount Path** to `/opt/render/project/src/uploads` (or just `uploads` depending on how your Express app resolves paths, but `/opt/render/project/src/uploads` is the safest absolute path if your code uses `path.join(__dirname, '../uploads')`). _Note: The Free tier does not support persistent disks. You will need the Starter tier ($7/mo) and a Disk ($1/mo) to prevent images from being deleted when the server restarts._
